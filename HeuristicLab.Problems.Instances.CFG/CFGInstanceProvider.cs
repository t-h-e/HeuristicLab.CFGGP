...
namespace HeuristicLab.Problems.Instances.CFG {
  public class CFGInstanceProvider : ProblemInstanceProvider<CFGData> {
...
In Proceedings of the 2015 on Genetic and Evolutionary Computation Conference (GECCO '15), Sara Silva (Ed.). ACM, New York, NY, USA, 1039-1046.";
    ...
    public override IEnumerable<IDataDescriptor> GetDataDescriptors() {
      ...
      using (var instanceStream = new ZipArchive(GetType().Assembly.GetManifestResourceStream(instanceArchiveName), ZipArchiveMode.Read)) {
        foreach (var entry in instanceStream.Entries.Where(
          ...
          yield return new CFGDataDescriptor(Path.GetFileNameWithoutExtension(entry), GetDescription(), entry);
        }
      }
    }
    ...
    public override CFGData LoadData(IDataDescriptor id) {
      ...
      var descriptor = (CFGDataDescriptor)id;
      ...
      using (var instancesZipFile = new ZipArchive(GetType().Assembly.GetManifestResourceStream(instanceArchiveName), ZipArchiveMode.Read)) {
        ...
        string tmpBnfFilename = Path.GetTempFileName() + ".bnf";
        entries.Where(x => x.Name.EndsWith(".bnf")).First().ExtractToFile(tmpBnfFilename);
        cfgData.Grammar = tmpBnfFilename;

        var embedEntry = entries.Where(x => x.Name.EndsWith("Embed.txt")).FirstOrDefault();
        if (embedEntry != null) {
          string tmpEmbedFilename = Path.GetTempFileName() + ".txt";
          embedEntry.ExtractToFile(tmpEmbedFilename);
          cfgData.Embed = tmpEmbedFilename;
        }
        ...
        using (StreamReader stream = new StreamReader(data.Open())) {
          ...
          if (int.TryParse(train[0], out help)) { cfgData.TrainStart = help; } else { throw new ArgumentException("Expected int."); }
          if (int.TryParse(train[1], out help)) { cfgData.TrainEnd = help; } else { throw new ArgumentException("Expected int."); }
          var test = stream.ReadLine().Split(new char[] { '-' });
          if (int.TryParse(test[0], out help)) { cfgData.TestStart = help; } else { throw new ArgumentException("Expected int."); }
          if (int.TryParse(test[1], out help)) { cfgData.TestEnd = help; } else { throw new ArgumentException("Expected int."); }
